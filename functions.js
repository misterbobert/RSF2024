...
{
    let output='';
    // Funcție de permutare simplificată
    ...
    // Funcție de criptare DES simplificată
    function desEncrypt(input, key) {
        ...
        let permutedInput = permute(input, initialPermutation);
        
        // Separarea în stânga și dreapta
        let left = permutedInput.substring(0, 32);
        let right = permutedInput.substring(32);

        // Cheia va fi folosită direct (fără generare de chei)
        let roundKey = key;

        // Aplicați o rundă de Feistel (simplificată)
        let newRight = xor(left, roundKey);
        let newLeft = right;

        // Refacem ieșirea
        let result = newLeft + newRight;

        // Permutare finală
        const finalPermutation = [40, 8, 48, 16, 56, 24, 64, 32,
        ...
        return permute(result, finalPermutation);
    }

    // Funcție pentru a converti un șir de caractere în binar (ASCII -> binar)
    ...
    // Funcție principală
    function main() {
        let inputText = inputOut; // Mesaj de 8 caractere
        let keyText = "12345678";   // Cheie de 8 caractere (64 de biți)

        // Convertim textul și cheia în binar
        let binaryInput = stringToBinary(inputText);
        let binaryKey = stringToBinary(keyText);

        // Asigurăm că inputul și cheia sunt exact 64 de biți
        binaryInput = binaryInput.padEnd(64, '0').slice(0, 64);
        binaryKey = binaryKey.padEnd(64, '0').slice(0, 64);
        ...
        if(state === 'cript')
        {
            encryptedBinary = desEncrypt(binaryInput, binaryKey);
            decryptedBinary = desEncrypt(encryptedBinary, binaryKey); // Folosim din nou funcția pentru simplitate
        }
        else if(state === 'decript')
        {
            decryptedBinary = desEncrypt(stringToBinary(inputText), binaryKey);
        }

        if(state === 'cript') output = encryptedBinary;
        else if(state === 'decript') output = binaryToString(decryptedBinary);    
    }

    // Executăm funcția principală
    main();

    document.getElementById('desOutput').value = output;
}